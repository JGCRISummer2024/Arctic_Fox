...
test$modis <- scales::rescale(test_modis$`MOD17A3H_Y_NPP_2023-01-01_rgb_3600x1800`, to = c(50, 2000))

#srdb data
srdb_modis <- terra::extract(modis_clamp, srdb[15:14])
srdb$modis <- scales::rescale(srdb_modis$`MOD17A3H_Y_NPP_2023-01-01_rgb_3600x1800`, to = c(50, 2000))
modisMap <- ggplot() + 
...
modisCheck <- ggplot(srdb, aes(ANPP, modis)) + geom_point()
